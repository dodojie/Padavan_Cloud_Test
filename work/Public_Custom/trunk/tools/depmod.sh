...
if [ "$CONFIG_FIRMWARE_INCLUDE_SQM" = "y" ] ; then
	sqm_dir="${ROOTDIR}/linux-3.4.x/net/sched"
	mkdir -p "${INSTALL_MOD_PATH}/lib/modules/${KERNELRELEASE}/kernel/net/sched"
	cp -f "$sqm_dir/"*.ko "${INSTALL_MOD_PATH}/lib/modules/${KERNELRELEASE}/kernel/net/sched"
	cp -f "${ROOTDIR}/linux-3.4.x/drivers/net/"*.ko "${INSTALL_MOD_PATH}/lib/modules/${KERNELRELEASE}/kernel/drivers/net/"
fi
