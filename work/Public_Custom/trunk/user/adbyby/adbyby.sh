...
files="/etc/storage/adbyby_*.sh"
file1="/etc/storage/adbyby.md5"
file2="/tmp/adbyby.md5"
...
Add_rules()
{
    if [ $adbyby_own_rule -eq 0 ] ; then
        ...
        if [ "$md5_json"x = x ] ; then
            ...
            [ "$lazy_txt"x != x ] && Download.sh /tmp/lazy_online_md5 ${lazy_txt}.md5 "" 1
            lazy_online=$(cat /tmp/lazy_online_md5)
            [ "$video_txt"x != x ] && Download.sh /tmp/video_online_md5 ${video_txt}.md5 "" 1
            ...
            touch /tmp/md5.json && Download.sh /tmp/md5.json $md5_json "" 1
        ...
        if [ "$lazy_online"x != "$lazy_local"x -o "$video_online"x != "$video_local"x ] ; then
            ...
            lazy_local=$(grep 'lazy' /tmp/local-md5.json | awk -F' ' '{print $1}')
            video_local=$(grep 'video' /tmp/local-md5.json | awk -F' ' '{print $1}')
            [ "$lazy_online"x == "$lazy_local"x ] && [ -s /tmp/lazy.txt ] && mv /tmp/lazy.txt /tmp/adbyby/data/lazy.txt && logger -t "adbyby" "静态规则 已更新！"
            [ "$video_online"x == "$video_local"x ] && [ -s /tmp/video.txt ] && mv /tmp/video.txt /tmp/adbyby/data/video.txt && logger -t "adbyby" "视频规则 已更新！"
       else
           echo "MD5 match! No need to update!"
           logger -t "adbyby" "静态规则┃视频规则 无需更新"
        fi
...
address=/dm.toutiao.com/0.0.0.0
