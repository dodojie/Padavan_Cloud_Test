...
adbyby_own_rule=`nvram get adbyby_own_rule`
md5_json_1=`nvram get md5_json_1`
lazy_txt_1=`nvram get lazy_txt_1`
video_txt_1=`nvram get video_txt_1`
md5_json_2=`nvram get md5_json_2`
lazy_txt_2=`nvram get lazy_txt_2`
video_txt_2=`nvram get video_txt_2`
...
if [ "$adbyby_hosts" = "1" ] && [ -s $hosts_bak ] ; then
...
Add_rules()
{
    if [ $adbyby_own_rule -eq 0 ] ; then
         logger -t "adbyby" "未启用内置的 静态规则┃视频规则"
         rm -f  /tmp/adbyby/data/lazy.txt /tmp/adbyby/data/video.txt /tmp/lazy.txt /tmp/video.txt /tmp/local-md5.json /tmp/md5.json
         nvram set adbyby_ltime="未启用"
         nvram set adbyby_vtime="未启用"
    else
        logger -t "adbyby" "Checking 静态规则┃视频规则"
        case $adbyby_own_rule in
         1)
            logger -t "adbyby" "使用【 默认 】内置规则"
            md5_json=$md5_json_1
            lazy_txt=$lazy_txt_1
            video_txt=$video_txt_1
            ;;
        2)
            logger -t "adbyby" "使用【 自定义 】内置规则"
            md5_json=$md5_json_2
            lazy_txt=$lazy_txt_2
            video_txt=$video_txt_2
            ;;
        esac
        # 去除 md5_json 中的空格
        md5_json=`echo ${md5_json// /}`
        touch /tmp/local-md5.json && md5sum /tmp/adbyby/data/lazy.txt /tmp/adbyby/data/video.txt > /tmp/local-md5.json
        if [ "$md5_json"x = x ] ; then
            logger -t "adbyby" "md5.json为空，尝试自动匹配……"
            wget --spider ${lazy_txt}.md5 2>/dev/null
            [ $? = "0" ] && Download.sh /tmp/lazy_online_md5 ${lazy_txt}.md5 "" 0
            lazy_online=$(cat /tmp/lazy_online_md5)
            wget --spider ${video_txt}.md5 2>/dev/null
            [ $? = "0" ] && Download.sh /tmp/video_online_md5 ${video_txt}.md5 "" 0
            video_online=$(cat /tmp/video_online_md5)
            [ ！-s /tmp/lazy_online_md5 ] && [ ！-s /tmp/video_online_md5 ] && logger -t "adbyby" "未获取对应的md5文件，自动匹配失败，终止Adbyby启动流程"  && exit
        else
            wget --spider $md5_json 2>/dev/null
            [ $? = "1" ] && logger -t "adbyby" "服务器md5_json文件不存在/服务器网络不通，终止Adbyby启动流程"  && exit
            touch /tmp/md5.json && Download.sh /tmp/md5.json $md5_json "" 0
        ...
        if [ "$lazy_online"x != "$lazy_local"x -o "$video_online"x != "$video_local"x ] ; then
            ...
            logger -t "adbyby" "获取md5成功！更新 静态规则┃视频规则 ..."
            wget --spider $lazy_txt 2>/dev/null
            [ $? = "0" ] &&  Download.sh /tmp/lazy.txt $lazy_txt
            wget --spider $video_txt 2>/dev/null
            [ $? = "0" ] && Download.sh /tmp/video.txt $video_txt
            # touch /tmp/lazy.txt && Download.sh /tmp/lazy.txt $lazy_txt
            # touch /tmp/video.txt && Download.sh /tmp/video.txt $video_txt
            touch /tmp/local-md5.json && md5sum /tmp/lazy.txt /tmp/video.txt > /tmp/local-md5.json
            lazy_local=$(grep 'lazy' /tmp/local-md5.json | awk -F' ' '{print $1}')
            video_local=$(grep 'video' /tmp/local-md5.json | awk -F' ' '{print $1}')
            [ "$lazy_online"x == "$lazy_local"x ] && mv /tmp/lazy.txt /tmp/adbyby/data/lazy.txt && logger -t "adbyby" "静态规则 已更新！"
            [ "$video_online"x == "$video_local"x ] && mv /tmp/video.txt /tmp/adbyby/data/video.txt && logger -t "adbyby" "视频规则 已更新！"
       else
           echo "MD5 match! No need to update!"
           logger -t "adbyby" "静态规则┃视频规则 已是最新"
        fi
    ...
    logger -t "adbyby" "Loading 黑名单┃白名单┃自定义规则"
    ...
    if [ $adbyby_rules_x -eq 1 ] ; then
        ...
        do
            ...
            if [ $rules_road -ne 0 ] ; then
                wget --spider $rules_address 2>/dev/null
                [ $? = "0" ] && Download.sh /tmp/adbyby/user2.txt $rules_address
                if [ -s "/tmp/adbyby/user2.txt" ] ; then
                   ...
                   logger -t "adbyby" "第三方规则下载失败：$rules_address！"
                   flag_rules_bak=0
                fi
...
EOF
        ...
        logger -t "adbyby" "anti_AD下载地址为空，请填写下载地址"
...
EOF
        ...
        logger -t "adbyby" "hosts下载列表为空，请填写下载地址"
