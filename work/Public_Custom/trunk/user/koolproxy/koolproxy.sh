...
koolproxyfile="$/etc_ro/koolproxy.tar.gz"
...
koolproxy_mount () {
	...
	if [ -s /etc_ro/koolproxy.tar.gz ] && [ ! -s "/tmp/koolproxy/koolproxy" ] ; then
		logger -t "【koolproxy】" "使用内置主程序"
		tar -xzvf "/etc_ro/koolproxy.tar.gz" -C "/tmp"
    fi
export PATH='/tmp/koolproxy:/etc/storage/bin:/tmp/script:/etc/storage/script:/opt/usr/sbin:/opt/usr/bin:/opt/sbin:/opt/bin:/usr/local/sbin:/usr/sbin:/usr/bin:/sbin:/bin'
chmod 777 /tmp/koolproxy/koolproxy
mkdir -p /tmp/koolproxy/data/
[[ "$(/tmp/koolproxy/koolproxy -h | wc -l)" -lt 2 ]] && rm -rf /tmp/koolproxy/koolproxy
}
...
if [ -s /tmp/koolproxy/data/rules/koolproxy.txt ] ; then
nvram set koolproxy_rules_date_local="`sed -n '1,10p' /tmp/koolproxy/data/rules/koolproxy.txt | grep "$(sed -n '1,10p' /tmp/koolproxy/data/rules/koolproxy.txt | grep -Eo '[0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+|201?.{1}' | sed -n '1p')" | sed 's/[x!]//g' | sed -r 's/-{2,}//g' | sed -r 's/\ {2}//g' | sed -r 's/\ {2}//g' | sed -n '1p'`"
nvram set koolproxy_rules_nu_local="`cat /tmp/koolproxy/data/rules/koolproxy.txt | grep -v ! | wc -l`"
nvram set koolproxy_video_date_local="`sed -n '1,10p' /tmp/koolproxy/data/rules/koolproxy.txt | grep "$(sed -n '1,10p' /tmp/koolproxy/data/rules/koolproxy.txt | grep -Eo '[0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+|201?.{1}' | sed -n '2p')" | sed 's/[x!]//g' | sed -r 's/-{2,}//g' | sed -r 's/\ {2}//g' | sed -r 's/\ {2}//g' | sed -n '1p'`"
nvram set koolproxy_h="`/tmp/koolproxy/koolproxy -h | awk 'NR==1{print}'`】【`sed -n '1,10p' /tmp/koolproxy/data/rules/daily.txt | grep "$(sed -n '1,10p' /tmp/koolproxy/data/rules/daily.txt | grep -Eo '[0-9]+-[0-9]+-[0-9]+ [0-9]+:[0-9]+|201?.{1}' | sed -n '1p')" | sed 's/[x!]//g' | sed -r 's/-{2,}//g' | sed -r 's/\ {2}//g' | sed -r 's/\ {2}//g' | sed -n '1p'`"
fi
...
[ ! -s "/tmp/koolproxy/koolproxy" ] && logger -t "【koolproxy】" "重新启动" && koolproxy_restart
if [ ! -f /tmp/cron_adb.lock ] ; then
	...
	if [ "$check" == "200" ] && [ ! -f /tmp/cron_adb.lock ] ; then
		reb=1
		PIDS=$(ps -w | grep "/tmp/koolproxy/koolproxy" | grep -v "grep" | wc -l)
		if [ "$PIDS" = 0 ] ; then 
			...
			sleep 3
			cd /tmp/koolproxy/
			/tmp/koolproxy/koolproxy $mode_video -d # >/dev/null 2>&1 &
			sleep 20
			reb=`expr $reb + 1`
		fi
		if [ "$PIDS" -gt 2 ] ; then 
...
rm -f /tmp/koolproxy.tar.gz /tmp/cron_adb.lock /tmp/sh_ad_kp_keey_k.sh /tmp/cp_rules.lock
...
#	if [ ! -s "/tmp/koolproxy/koolproxy" ] ; then
#		logger -t "【koolproxy】" "下载失败, 注意检查端口是否有冲突,程序是否下载完整,10 秒后自动尝试重新启动" && sleep 10 && koolproxy_restart x
#	fi
	# 恢复上次保存的证书
	mkdir -p /etc/storage/koolproxy /tmp/koolproxy/data/certs /tmp/koolproxy/data/private
	[ -f /etc/storage/koolproxy/base.key.pem ] && cp -f /etc/storage/koolproxy/base.key.pem /tmp/koolproxy/data/private/base.key.pem
	[ -f /etc/storage/koolproxy/ca.key.pem ] && cp -f /etc/storage/koolproxy/ca.key.pem /tmp/koolproxy/data/private/ca.key.pem
	[ -f /etc/storage/koolproxy/ca.crt ] && cp -f /etc/storage/koolproxy/ca.crt /tmp/koolproxy/data/certs/ca.crt
	cd /tmp/koolproxy/data/
	if [ ! -f /tmp/koolproxy/data/private/ca.key.pem ] ; then
		logger -t "【koolproxy】" "检测到首次运行https，开始生成koolproxy证书，用于https过滤！"
		chmod 777 /tmp/koolproxy/data/gen_ca.sh && sh gen_ca.sh
		# 保存证书
		[ -f /tmp/koolproxy/data/certs/ca.crt ] && cp -f /tmp/koolproxy/data/certs/ca.crt /etc/storage/koolproxy/ca.crt
		[ -f /tmp/koolproxy/data/private/base.key.pem ] && cp -f /tmp/koolproxy/data/private/base.key.pem /etc/storage/koolproxy/base.key.pem
		[ -f /tmp/koolproxy/data/private/ca.key.pem ] && cp -f /tmp/koolproxy/data/private/ca.key.pem /etc/storage/koolproxy/ca.key.pem && mtd_storage.sh save &
	fi
	touch index.txt
	echo 1000 > serial
...
#	fi
	source_list="/tmp/koolproxy/data/source.list"
...
#		sed -Ei "s@^0\|daily.txt@1\|daily.txt@"  $source_list
	...
	if [ "$adbyby_adblocks" = "1" ] ; then
		...
		rm -f /tmp/koolproxy/user3adblocks.txt
		while read line
		do
		c_line=`echo $line |grep -v "#"`
		if [ ! -z "$c_line" ] ; then
			logger -t "【koolproxy】" "第三方规则:$line"
			#wgetcurl.sh /tmp/koolproxy/user2.txt $line $line N
			wget --no-check-certificate $line -O /tmp/koolproxy/user2.txt
			grep -v '^!' /tmp/koolproxy/user2.txt | grep -E '^(@@\||\||[[:alnum:]])' | sort -u | grep -v "^$" >> /tmp/koolproxy/user3adblocks.txt
			rm -f /tmp/koolproxy/user2.txt
		fi
		done < /tmp/rule_DOMAIN.txt
	fi

	# 合并规则
	cat /etc/storage/koolproxy_rules_script.sh | grep -v '^!' | grep -v "^$" > /tmp/koolproxy/user.txt
	grep -v '^!' /tmp/koolproxy/user3adblocks.txt | grep -v "^$" >> /tmp/koolproxy/user.txt
	rm -f /tmp/koolproxy/user3adblocks.txt
	ln -sf /tmp/koolproxy/user.txt /tmp/koolproxy/data/user.txt
	ln -sf /tmp/koolproxy/user.txt /tmp/koolproxy/data/rules/user.txt
	cd /tmp/koolproxy/
	...
	chmod 777 /tmp/koolproxy/koolproxy
	export PATH='/tmp/koolproxy:/etc/storage/bin:/tmp/script:/etc/storage/script:/opt/usr/sbin:/opt/usr/bin:/opt/sbin:/opt/bin:/usr/local/sbin:/usr/sbin:/usr/bin:/sbin:/bin'
	export LD_LIBRARY_PATH=/tmp/koolproxy/lib:/lib:/opt/lib
	nvram set koolproxy_h="`/tmp/koolproxy/koolproxy -h | awk 'NR==1{print}'`"
	cd /tmp/koolproxy/
	/tmp/koolproxy/koolproxy $mode_video -d # >/dev/null 2>&1 &
	rm -f /tmp/adbyby_host.conf
	sleep 10
...
koolproxy_add_rules
rm -f /tmp/koolproxy.tar.gz /tmp/cron_adb.lock
/etc/storage/ez_buttons_script.sh 3 & #更新按钮状态
...
[ ! -f /tmp/koolproxy/data/rules/koolproxy.txt ] && return 0
# Koolproxy Rules to Domain List
rm -f /tmp/koolproxy/domain.txt /tmp/koolproxy/domain2.txt /tmp/koolproxy/ip.txt
cd /tmp/koolproxy/data/rules
# while read line
# do
...
krdl ./daily.txt
sleep 2
eval $(ls| grep txt.http| awk '{print "cat /tmp/koolproxy/data/rules/"$1" >> /tmp/koolproxy/domain.txt;";}')
# 提取IP
cat  /tmp/koolproxy/domain.txt /tmp/koolproxy/koolproxy_blockip.txt | grep -Eo '^[0-9\.]*$' | sort -u | grep -v "^$" > /tmp/koolproxy/ip.txt
cat /tmp/koolproxy/ip.txt /tmp/koolproxy/koolproxy_blockip.txt | sort -u > /tmp/koolproxy/koolproxy_blockip.txt
sed -Ei '/0.0.0.0/d' /tmp/koolproxy/koolproxy_blockip.txt
# 提取Domain
cat  /tmp/koolproxy/domain.txt | grep  -Ev '^[0-9\.]*$' | sort -u > /tmp/koolproxy/domain2.txt
sed -e "s/^/ipset=\/\./" -e "s/$/\/black_koolproxy/" -i /tmp/koolproxy/domain2.txt
cat /tmp/koolproxy/domain2.txt /tmp/koolproxy/data/koolproxy_ipset.conf | sort -u > /tmp/adbyby_host.conf
# 删tmp
rm -f "/tmp/koolproxy/data/rules/*.txt.http" "/tmp/koolproxy/data/rules/*.txt.https"
rm -f /tmp/koolproxy/domain.txt /tmp/koolproxy/domain2.txt /tmp/koolproxy/ip.txt
}
...
[ ! -f /tmp/adbyby_host.conf ] && [ -f /tmp/koolproxy/data/koolproxy_ipset.conf ] && cp -f /tmp/koolproxy/data/koolproxy_ipset.conf /tmp/adbyby_host.conf
...
if [ -f /tmp/koolproxy/koolproxy_blockip.txt ] ; then
	grep -v '^#' /tmp/koolproxy/koolproxy_blockip.txt | sort -u | grep -v "^$" | sed -e "s/^/-A adbybylist &/g" | ipset -R -!
fi
restart_dhcpd
...
for host in $line; do
	...
	if [ -f /tmp/koolproxy/koolproxy_hookport.txt ] && [ "$adm_hookport" == 1 ] ; then
		hookport443='|'
		sed -e "s/443$hookport443//" -i /tmp/koolproxy/koolproxy_hookport.txt
		i=1 && hookport1="" && hookport2="" && hookport3="" && hookport4="" && hookport5=""
		for hookport in $(cat /tmp/koolproxy/koolproxy_hookport.txt | sed s/\|/\ /g)
		do
