...
func_start(){
	/usr/bin/vlmcsd -i /etc/storage/vlmcsdini_script.sh -v -l syslog
	...
	[ -z "$(ps -w | grep "vlmcsd" | grep -v grep )" ] && logger -t "【kms】" "启动失败！注意检查kms端口（默认1688）是否有冲突。" && nvram set vlmcsd_enable="0"
}

func_stop(){
	sed -Ei '/_vlmcs._tcp/d' /etc/storage/dnsmasq/dnsmasq.conf; restart_dhcpd;
	killall -q vlmcsd vlmcsdini_script.sh
...
#使用VPN适配器“KMS Mirror”为其提供IP地址192.168.3.100，租期为一天，并使整个192.168.128.x成为隐藏的本地IPv4地址。
;VPN = KMS Mirror=192.168.3.100/24:1d
