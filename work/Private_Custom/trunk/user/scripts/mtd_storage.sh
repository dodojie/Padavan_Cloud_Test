...
func_fill()
{
	...
	# Wifi自动中继脚本
	ap_script="$dir_storage/ap_script.sh"

	# 自定义CPU分配脚本
	script_cpu="$dir_storage/script/cpu_custom.sh"

	# 日志提示实际CPU分配
	script_display="$dir_storage/script/log_display.sh"

	# 连接参数设置——默认
	script_default="$dir_storage/script/connect_default.sh"

	# 连接参数设置——激进
	script_optimize="$dir_storage/script/connect_optimize.sh"
...
############################################################
	# create custom dir
	[ ! -d "$dir_script" ] && mkdir -p -m 755 "$dir_script"

	# create cpu script
	if [ ! -f "$script_cpu" ] ; then
		cat > "$script_cpu" <<'EOF'
#!/bin/sh
##################################################
#        手动        CPU多核心分配               #
##################################################
logger -t "自定义Sh" "开始分配cpu负载..."
## 	PPPOE拨号
echo 1 >/sys/class/net/ppp0/queues/rx-0/rps_cpus
echo 4 >/sys/class/net/ppp0/queues/tx-0/xps_cpus

## 	WAN广域网
echo 1 >/sys/class/net/eth3/queues/rx-0/rps_cpus
echo 4 >/sys/class/net/eth3/queues/tx-0/xps_cpus

## 	Br0交换机
echo 1 >/sys/class/net/br0/queues/rx-0/rps_cpus
echo 1 >/sys/class/net/br0/queues/tx-0/xps_cpus

## 	LAN局域网 中断
echo 4 > /proc/irq/11/smp_affinity

## 	LAN有线
echo 4 >/sys/class/net/eth2/queues/rx-0/rps_cpus
echo 4 >/sys/class/net/eth2/queues/tx-0/xps_cpus
...
echo 2 >/sys/class/net/ra0/queues/tx-0/xps_cpus
...
echo 8 >/sys/class/net/rai0/queues/tx-0/xps_cpus

for device in $(ls /sys/class/net); do
	echo 16384 >/sys/class/net/$device/queues/rx-0/rps_flow_cnt
done
	echo 16384 >/proc/sys/net/core/rps_sock_flow_entries
logger -t "自定义Sh" "已完成cpu负载分配"
##################################################
EOF
		chmod 777 "$script_cpu"
	fi

	# create display script
	if [ ! -f "$script_display" ] ; then
		cat > "$script_display" <<'EOF'
#!/bin/sh
logger -t "网络监测" "中断_11_eth2" "运行于CPU: " "$(cat /proc/irq/11/smp_affinity)"
logger -t "网络监测" "中断_12_ra0 " "运行于CPU: " "$(cat /proc/irq/12/smp_affinity)"
logger -t "网络监测" "中断_32_rai0" "运行于CPU: " "$(cat /proc/irq/32/smp_affinity)"
for device in $(ls /sys/class/net); do
	logger -t "网络监测" "$device rx" "运行于CPU: " "$(cat /sys/class/net/$device/queues/rx-0/rps_cpus)"
	logger -t "网络监测" "$device tx" "运行于CPU: " "$(cat /sys/class/net/$device/queues/tx-0/xps_cpus)"
done
EOF
		chmod 777 "$script_display"
	fi

	# create default script
	if [ ! -f "$script_default" ] ; then
		cat > "$script_default" <<'EOF'
#!/bin/sh
logger -t "自定义Sh" "开始设置连接“常规”参数..."
# TCP SYN SENT 超时时间，默认值：20
echo 20 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_syn_sent

# TCP SYN RECV 超时时间，默认值：60
echo 60 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_syn_recv

# TCP ESTABLISHED 超时时间，默认值：180
echo 180 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_established

# TCP FIN WAIT 超时时间，默认值：120
echo 120 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_fin_wait

# TCP CLOSE WAIT 超时时间，默认值：60
echo 60 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_close_wait

# TCP LAST ACK 超时时间，默认值：30
echo 30 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_last_ack

# TCP TIME WAIT 超时时间，默认值：120
echo 120 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_time_wait

# TCP CLOSE 超时时间，默认值：10
echo 10 > /proc/sys/net/netfilter/nf_conntrack_tcp_timeout_close

# UDP 超时时间，默认值：180
echo 60 > /proc/sys/net/netfilter/nf_conntrack_udp_timeout

# UDP STREAM 超时时间，默认值：180
...
logger -t "自定义Sh" "已完成连接参数设置"
EOF
		chmod 777 "$script_default"
	fi

	# create optimize script
	if [ ! -f "$script_optimize" ] ; then
		cat > "$script_optimize" <<'EOF'
#!/bin/sh
logger -t "自定义Sh" "开始设置连接“激进”参数..."
...
if [ -s /etc/storage/dnsmasq/dnsmasq.conf ] ; then
    sed -i '/adbyby\/hosts/d' "/etc/storage/dnsmasq/dnsmasq.conf"
fi
...
###########################################
### 启用 “常规”连接参数
#/etc/storage/script/connect_default.sh set

### 启用 “激进”连接参数
### “激进”和“常规”同时开启，排在后面的生效
#/etc/storage/script/connect_optimize.sh set

### 启用手动 MT7621 CPU 分配
#/etc/storage/script/cpu_custom.sh set
###########################################
EOF
		chmod 777 "$script_started"
	fi
...
### $3 - WAN IPv4 address
if [ "$1" = "up" ] && [ -f /usr/bin/smartdns.sh ] ; then
    /usr/bin/smartdns.sh reset >/dev/null 2>&1
fi


EOF
		chmod 777 "$script_postw"
	fi
...
# 检查是否连上AP最大时长； 1=1秒， 2=2秒.... 
connect_ap=100

# 检查是否联网时最大时长；1=3秒，2=6秒.... (信号差建议设置成50)
connect_net=30
...
INTERNET_TIME=$2
if [ $INTERNET == 0 ];then
    hour=$(( $INTERNET_TIME/3600 ))
    min=$(( ($INTERNET_TIME-${hour}*3600)/60 ))
    sec=$(( $INTERNET_TIME-${hour}*3600-${min}*60 ))
    logger -t "【网络检测】" "已断开Internet互联网，本次联网持续 ${hour} 小时 ${min} 分 ${sec} 秒."
else
    logger -t "【网络检测】" "已连接到Internet互联网，耗时$INTERNET_TIME秒."
fi
        ...
        if [ ! -z "$c_line" ] ; then
            ...
            if [ ! -z "$site_survey" ] ; then
                ...
                if [ $Signal -lt $sig ]; then
                    logger -t "【连接 AP】" "跳过$rtwlt_sta_ssid；信号强度 $Signal%，低于设置的 $sig%"
                    ap3=1
                fi
            fi
            if [ "$apblack" = "1" ] ; then
            ...
            if [ "$ap3" != "1" ] ; then
                ...
                logger -t "【连接 AP】" "【中继模式:$rtwlt_mode_x $rtwlt_sta_wisp】【Security:$Security】【WMode:$WMode】"
                logger -t "【连接 AP】" "【AP:$rtwlt_sta_ssid】【密码:$rtwlt_sta_wpa_psk】【信号:$Signal%】【信道:$Ch】【BSSID:$BSSID】"
...
### 指定向上游查询的 UDP 端口，默认随机，开启后降低安全性、加快速度、减少资源占用
query-port=53535
...
### DNS解析转发的UDP最大数据包，默认4096
edns-packet-max=1280

### 设置DNS的查询并发的最大数量。默认值是150。
dns-forward-max=500

### 完整域名才向上游查询，主机名仅查找host
domain-needed

### 使用 SmartDNS 作为 dnsmasq 上游时，建议禁用以下自定义设置
### 顺序查询dns(/etc/resolv.conf和resolv-file）
#strict-order

### 并发查询中所有上游DNS服务器(/etc/resolv.conf和resolv-file）
#all-servers

### 循环检测（配合all-servers使用，防止不当设置循环查询dns）
#dns-loop-detect
...
l# Custom user conf file for inadyn DDNS client
